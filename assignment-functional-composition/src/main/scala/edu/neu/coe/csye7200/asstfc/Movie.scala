...
object Movie extends App {
  ...
  // Serialize the sequence of Movies into JSON
  val json = ms.toJson

  // Deserialize the JSON back into a sequence of Movies
  val deserializedMovies = json.convertTo[Seq[Movie]]

  // Check if the deserialized data is equal to the original data
